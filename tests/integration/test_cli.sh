...
assert_contains "Source path parsed" "$output" "Found 1 files in '$SOURCE_DIR'"
...
output1=$("$BINARY" -t hello --dry-run -v -s "$SOURCE_DIR" -d "$TARGET_DIR" 2>&1)
assert_contains "Tags before dirs accepted" "$output1" "Found 1 files"
assert_contains "Tag correctly parsed" "$output1" "hello"

output2=$("$BINARY" -s "$SOURCE_DIR" -v -d "$TARGET_DIR" -f -t hello --dry-run 2>&1)
...
output=$("$BINARY" -s "$SOURCE_DIR" -d "$TARGET_DIR" -- --dry-run 2>&1 || true)
assert_contains "-- stops option parsing" "$output" "Unexpected argument"

output=$("$BINARY" -s "$SOURCE_DIR" -- -d "$TARGET_DIR" 2>&1 || true)
assert_contains "-- prevents -d parsing" "$output" "--source and --target are required"

output=$("$BINARY" -s "$SOURCE_DIR" -d "$TARGET_DIR" --dry-run -- extra 2>&1 || true)
assert_contains "Positional after -- rejected" "$output" "Unexpected argument"
...
output=$("$BINARY" -s "$SOURCE_DIR" -d "$TARGET_DIR" --dry-run foo bar 2>&1 || true)
