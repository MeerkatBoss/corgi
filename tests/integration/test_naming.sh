...
test_group "Single tag"
    ...
    assert_matches "Tag in filename" \
        "$filename" \
        "^[0-9]{4}-[0-9]{2}-[0-9]{2}_[0-9]{3}_vacation\.jpg$"
finish_test || exit 1
...
test_group "Preserve extension"
    ...
    assert_file_exists "TXT file copied" \
        "$(find "$TARGET_DIR" -name "*_mixed.txt" | head -1)"
    assert_file_exists "JPG file copied (case-sensitive)" \
        "$(find "$TARGET_DIR" -name "*_mixed.JPG" | head -1)"
    assert_file_exists "JSON file copied" \
        "$(find "$TARGET_DIR" -name "*_mixed.json" | head -1)"
    assert_file_exists "No-extension file copied" \
        "$(find "$TARGET_DIR" -name "*_mixed" -type f ! -name "*.*" | head -1)"
finish_test || exit 1
